...
class TradeEngine:
    ...
    async def _emergency_close_leg(
        ...
        if close_order["status"] == "error":
            ...
            # ИСПРАВЛЕНО: добавлен pair_id, убран await (sync метод)
    ...
    async def execute_entry(
        ...
        logger.info(
            f"🚀 ENTRY TRY | LONG [{long_ex}] BUY {volume} | "
            f"SHORT [{short_ex}] SELL {volume} | {symbol}"
        )

        # ------------------------------------------------------------
        # ШАГ 1. Открываем LONG (с ретраями)
        # ------------------------------------------------------------
        long_order = await self._order_with_retries(
            long_ex,
            symbol,
            "buy",
            volume,
        ...
        if long_order["status"] == "error":
            logger.error(
                f"❌ ENTRY FAILED | LONG не открыт [{long_ex}] | {long_order.get('msg')}"
            )
            return {
                "success": False,
                "entry_long_order": long_order,
                "entry_short_order": None,
                "error": "long_leg_failed",
                "imbalance": None,
            }

        long_filled = long_order.get("filled") or 0.0
        
        # Проверяем, достаточно ли исполнено LONG
        if long_filled < volume * MIN_FILL_RATIO:
            logger.warning(
                f"⚠️ LONG частично исполнен [{long_ex}] | "
                f"filled={long_filled}, requested={volume}, ratio={long_filled/volume:.2%}"
            )
            # Пробуем докупить
        ...
        logger.info(
            f"🟩 LONG OPENED | {long_ex} BUY {volume} {symbol} | "
            f"status={long_order['status']} filled={long_filled}"
        )

        # ------------------------------------------------------------
        # ШАГ 2. Открываем SHORT (с ретраями)
        ...
        if short_order["status"] != "error":
            short_filled = short_order.get("filled") or 0.0
            
            # Проверяем, достаточно ли исполнено SHORT
            if short_filled < short_volume * MIN_FILL_RATIO:
                logger.warning(
                    f"⚠️ SHORT частично исполнен [{short_ex}] | "
                    f"filled={short_filled}, requested={short_volume}"
                )
                # Пробуем докупить
                short_order = await self._fill_remaining(
                    short_ex, symbol, "sell", short_volume, short_filled,
                    leg_label="entry_short",
                    params={"reduceOnly": False},
                )
            ...
            if imbalance_pct > CRITICAL_IMBALANCE_PCT:
                ...
                if close_result["critical"]:
                    # Не удалось закрыть избыток — критическая ситуация
                    logger.critical(
            ...
            # Успешный вход
            logger.success(
                f"✅ ENTRY SUCCESS | LONG={long_filled}, SHORT={short_filled}, "
        ...
        # SHORT не открылся — аварийное закрытие LONG
        logger.error(
        ...
        return {
            ...
            "entry_long_order": long_order,
            "entry_short_order": short_order,
            "error": "second_leg_failed_emergency_close",
            "imbalance": None,
        }
